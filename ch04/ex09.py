...
#오차 역전파(back propagation)의 개념 :: 미분의 연쇄법칙 -> 반대방향으로 미분값을 주겠다?
# 미분을 하나씩 뒤로 밀자 근데 졸려 죽겠다
# 미분의 연쇄법칙을 거꾸로 적용
# ex) f(x) = x의 -1승 미분 = -1/x**2
# f(x) = e ** x 미분해도 안바뀌고 그대로
# f(x) = log x -> 1 / x

# 게산그래프만으로 변화율을 전부 계산할 수 있다? 당연히 되지않남...
