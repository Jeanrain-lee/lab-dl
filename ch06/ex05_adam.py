...
파라미터 최적화 알고리즘 4) Adam(Adaptive Moment estimate)
    ...
    beta1, beta2: 모멘텀을 변화시킬 때 사용하는 상수들 #0에서 1사이 숫자
...
from dlf.ch06.ex01_matplot3d import fn, fn_derivative
...
class Adam:
    ...
    def update(self, params, gradients):
        self.t += 1  # update가 호출될 때마다 timestamp 1씩 증가
...
if __name__ == '__main__':
    ...
    # Adam 클래스의 인스턴스 생성
    adam = Adam(lr=0.3) # lr=0.01

    # 학습하면서 파라미터 (x, y)들이 업데이트되는 내용을 저장하기 위한 리스트 선언
    x_history = []
    y_history = []
    for i in range(800):
...
# ada의 단점을 수정한, moment의 단점을 개선한 각각의 알고 2
